...
from processing.infer import CompoundVectorInference
from processing.visualize import visualize_motion
...
def main(image_path: Optional[str], crop: Optional[float], maxwidth: Optional[int], step: Optional[float]):
    ...
    vector_inference = CompoundVectorInference()
    ...
    while True:  # Adjust the range according to your needs
        ...
        ### do it
        img, flow = processor.apply(frame)
        motion_vector = vector_inference.infer(flow)
        visualize_motion(img, motion_vector)
