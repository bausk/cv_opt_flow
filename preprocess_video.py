...
def main(image_path: str, step: Optional[float], crop: Optional[float], maxwidth: Optional[int], output: Optional[str]):
    """
    Usage:
    $ python preprocess_video.py --image_path=xxx.MP4 --step=50
    Will produce video and images timelapsed with 50 seconds interval
    """
    preprocessor = MP4VideoPreprocessor(image_path, increment=step, crop_factor=crop, resize_maxwidth=maxwidth)
